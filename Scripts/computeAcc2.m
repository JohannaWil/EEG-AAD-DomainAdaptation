function acc = computeAcc2(nTestTrials, mX, vClass, method,k,p)
...
% Number of trials
n = length(vClass);
start = n - nTestTrials + 1;
vS = 1:n;
ind = 1;
for s = start:n % For each candidate trial
    ...
    preAcc{1}(ind) = mean(Mdl.predict(mX2) == testClasses);

    %Decision tree
    Mdl = fitctree(mX1,trainClasses);
    preAcc{2}(ind) = mean(Mdl.predict(mX2) == testClasses);
        
    %k-Nearest Neighbour    
    for kk = 1:length(k)
        Mdl = fitcknn(mX1,trainClasses,'NumNeighbors',k(kk));
        preAcc{2+kk}(ind) = mean(Mdl.predict(mX2) == testClasses);
    end
    ind = ind + 1;   
